...
export default function LoginPage(
  props: IDarkThemeProps & { setAuthenticated: (success: boolean) => void },
) {
  ...
  const password2Warning =
    visited.password2 &&
    (details.password2.trim() === '' || details.password !== details.password2);
  const password2WarningText =
    visited.password2 && details.password2.trim() === '' ? t('required') : t('noMatch');
    ...
    if (login) {
      ...
      createAccount(details.username, details.password).then(async (created) => {
        if (created === true) {
          ...
          AppToasterTop.show({
            message: t('accountErrorServer', { message: created }),
            intent: 'warning',
          });
        } else {
  ...
  return (
    <>
      {mobile && <MobileHeader darkThemeProps={props} className="login-header" />}
      <div className="body">
        ...
        <div className="main-content">
          <div className="login">
            ...
            {wrongCredentials && (
              <Callout intent="danger" className="error-callout" icon={null}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              />
              {
                <div className="toggle-login">
                  <div className={Classes.TEXT_MUTED}>{login ? t('new') : ''}</div>
                  <div

...
app.get(['/uniqueRecipes/*slug', '/recipes/*slug'], async (req, res) => {
  ...
  const result = await fetch(url, { headers: { "Express-Secret": expressSecret } });
  if (result.ok) {
    const recipe = await result.json() as IRecipe;
    let index = await readFileAsync(resolve(staticDir, 'index.html'), 'utf-8');

    // replace title
    index = index.replaceAll(/(<meta\s+(?:property|name)="[^"]*title"\s+content=")[^"]*(">)/g,
      (_, p1, p2) => `${p1}${recipe.title}${p2}`)
      .replace(/(<title>)[^>]*<\/title>/, `<title>${recipe.title}</title>`);

    // replace description
    index = index.replaceAll(/(<meta\s+(?:property|name)="[^"]*description"\s+content=")[^"]*(">)/g,
    ...
    index = index.replaceAll(/(<meta\s+(?:property|name)="[^"]*url"\s+content=")[^"]*(">)/g,
      (_, p1, p2) => `${p1}${uniqueUrl}${p2}`);

    // replace image
    if (recipe.image.length > 0) {
      index = index.replaceAll(/(<meta\s+(?:property|name)="[^"]*image"\s+content=")[^"]*(">)/g,
